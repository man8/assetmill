...
import { SourceImage, AssetVariant, ProcessingOptions, GeneratedAsset, MarginConfig, PipelineConfig } from '../types';
...
export class ImageProcessor {
  ...
  static async processImage(
    ...
    try {
      ...
      } else if (variant.theme === 'monochrome' || options.theme === 'monochrome') {
        pipeline = pipeline.greyscale();
        
        const themeConfig = config?.processing?.themes?.monochrome;
        if (themeConfig?.colorTransforms && themeConfig.colorTransforms.length > 0) {
          for (const transform of themeConfig.colorTransforms) {
            if (transform.from === '#000000' && 
                (transform.to === '#ffffff' || transform.to.toLowerCase() === 'white')) {
              pipeline = pipeline.negate({ alpha: false });
              break;
            }
          }
      ...
      switch (variant.format) {
        ...
        case 'avif':
          ...
          break;
        case 'ico':
          return await this.generateMultiSizeIcoFile(sourceImage, variant, outputPath);
        default:
  ...
  static async generateMultiSizeIcoFile(
    ...
    const sizes = standardSizes.includes(requestedSize) 
      ? standardSizes 
